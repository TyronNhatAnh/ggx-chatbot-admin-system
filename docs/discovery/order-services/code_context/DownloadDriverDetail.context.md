...
```go
{
	...
	resp, err := h.statementOfUseService.GetDriverDetail(c.Request.Context(), request)
	if err != nil {
		c.JSON(http.StatusBadRequest, httpcommon.NewErrorResponse(err.Error(), httpcommon.RequestInvalid, ""))
		return
	}

	url, err := h.statementOfUseService.ExportToExcelDriverDetail(c.Request.Context(), resp, request)
	if err != nil {
		c.JSON(http.StatusBadRequest, httpcommon.NewErrorResponse(err.Error(), httpcommon.RequestInvalid, ""))
		return
	}
...
- `statementOfUseService.ExportToExcelDriverDetail()`
