...
### Session 2 — Order List & Filter (`get_orders_admin_panel`)
...
### Session 6 — User Lookup (`get_user_profile`, `search_users`)
...
### Session 7 — Organization (`get_organization_by_id`, `search_organizations`)
1. What is the info for organization ID 10?
2. Search for an organization named "GoGoX"
3. List all B2B organizations in the system
4. Which organizations have "Logistics" in their name?
5. How many branches does org ID 99 have?

### Session 8 — Branch (`get_branch_by_id`, `search_branches`)
1. What is the name of branch ID 55 and which org does it belong to?
2. Find a branch with "Ha Noi" in the name
3. List all branches under org ID 10
4. Which branches have "HCM" in their name?
5. *(follow-up from Q2)* What is the address of that branch?
...
1. What is the profile of driver ID 7777?
2. Find a driver with name or phone "0912345678"
3. What is the orderCap and creditAmount of driver "Le Van D"?
4. Search for drivers named "Minh" — how many results come back?
5. What level and vehicle type does driver ID 1234 have?
...
1. Calculate the fare for driver ID 7777 on order ID 123456
2. How much will driver ID 1234 earn from order 654321?
3. What does the fare breakdown look like for driver 9999 on order 111111?
4. How much VAT is included in the fare for driver 7777 on order 123456?
...
1. Search for the address "123 Nguyen Hue" in the system
2. Search external addresses with keyword "Hoan Kiem Ha Noi"
3. Find saved addresses for user ID 10001 with keyword "district 1"
4. Get detailed address results for "Lotte Center Ha Noi"
5. What addresses match the keyword "Tan Binh HCM"?
...
### Session 17 — Struct & Code Search (`get_struct_definition`, `search_codebase`)
1. What fields does the `B2COrderDetail` struct have?
2. What does the `Order` struct look like in the backend?
3. Search the codebase: "how is pricing calculated?"
4. Which struct contains the `appointmentAt` field?
5. Search for: "order cancellation validation logic"

### Session 18 — Graph Traversal & Flow Tracing (`trace_service_flow`, `find_api_consumers`, `trace_full_stack`, `traverse_graph`, `get_knowledge_stats`)
1. Trace the service flow for the `GetOrderDetail` handler
2. Which frontend pages call the API `/orders/:orderId`?
3. Trace the full stack for the `SubmitOrder` endpoint
...
2. Who is driver ID 7777? Do they have any active orders right now?
3. Who is the customer and org for order `ORD-20240315-001`?
...
1. List available docs — how many handlers have been indexed?
2. *(follow-up)* From that list, which handlers are related to "driver"?
3. *(follow-up)* Show the handler context for the first driver-related handler
4. Is business registration number `1234567890` valid?
5. *(wrap-up)* Summarize: how many order types and payment methods does the system support?
...
|---------|--------------|
| 1 | `get_order_detail` |
| 2 | `get_orders_admin_panel` |
| 3 | `get_order_payment_status`, `get_order_cancel_fee` |
| 4 | `get_order_history` |
| 5 | Multi-turn order memory |
| 6 | `get_user_profile`, `search_users` |
| 7 | `get_organization_by_id`, `search_organizations` |
| 8 | `get_branch_by_id`, `search_branches` |
...
| 17 | `get_struct_definition`, `search_codebase` |
| 18 | `trace_service_flow`, `find_api_consumers`, `trace_full_stack`, `traverse_graph`, `get_knowledge_stats` |
